...
test('PlannedTransformation', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const etswehlcpb = new SKOSConcept({ connector, semanticId: 'http://base.com/befemcqdvd' });
		const tdiefhylli = [new PlannedConsumptionFlow({ connector, semanticId: 'http://base.com/ufmvkahhcd' })];
		const nahvgqqhwp = [new PlannedProductionFlow({ connector, semanticId: 'http://base.com/xgbkvlcbtn' })];
		const obj = new PlannedTransformation({
			connector,
			semanticId: "http://example.org/obj",
			transformationType: etswehlcpb,
			consumptionFlows: tdiefhylli,
			productionFlows: nahvgqqhwp
		});

		assert.strictEqual('http://example.org/obj', obj.getSemanticId());

		const actualTransformationType = await obj.getTransformationType();
		const expectedTransformationType = etswehlcpb;
		...
		const expectedConsumptionFlows = tdiefhylli;
		...
		const expectedProductionFlows = nahvgqqhwp;
