...
test('PlannedTransformation', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const wfgpqtganv = new SKOSConcept({ connector, semanticId: 'http://base.com/hgwtkohiaw' });
		const eqidvabqhs = [new PlannedConsumptionFlow({ connector, semanticId: 'http://base.com/tfnmkadcbc' })];
		const jvvmxwitdu = [new PlannedProductionFlow({ connector, semanticId: 'http://base.com/paxtzniulw' })];
		const obj = new PlannedTransformation({
			connector,
			semanticId: "http://example.org/obj",
			transformationType: wfgpqtganv,
			consumptionFlows: eqidvabqhs,
			productionFlows: jvvmxwitdu
		});

		assert.strictEqual('http://example.org/obj', obj.getSemanticId());

		const actualTransformationType = await obj.getTransformationType();
		const expectedTransformationType = wfgpqtganv;
		...
		const expectedConsumptionFlows = eqidvabqhs;
		...
		const expectedProductionFlows = jvvmxwitdu;
