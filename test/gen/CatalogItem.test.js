...
test('CatalogItem', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const ponhdjfhkl = new SuppliedProduct({ connector, semanticId: 'http://base.com/icbsbdwemi' });
		
		
		const tmwbtfrzzi = [new Offer({ connector, semanticId: 'http://base.com/gspxnkhsns' })];
		const hwqbwmzajg = [new Catalog({ connector, semanticId: 'http://base.com/qzrceqblhr' })];
		const obj = new CatalogItem({
			connector,
			semanticId: "http://example.org/obj",
			product: ponhdjfhkl,
			sku: "jwcxlwmehk",
			stockLimitation: 0.43232954,
			offers: tmwbtfrzzi,
			catalogs: hwqbwmzajg
		});
		...
		const expectedOffers = tmwbtfrzzi;
		...
		const expectedCatalogs = hwqbwmzajg;
