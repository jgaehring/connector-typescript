...
test('Offer', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const vtpmgzzbmv = new CatalogItem({ connector, semanticId: 'http://base.com/wuewgcrrpb' });
		const gopfjmeqrm = new CustomerCategory({ connector, semanticId: 'http://base.com/sekaaflhpk' });
		const trmhpbbycz = new Price({ connector });
		
		const obj = new Offer({
			connector,
			semanticId: "http://example.org/obj",
			offeredItem: vtpmgzzbmv,
			offeredTo: gopfjmeqrm,
			price: trmhpbbycz,
			stockLimitation: 0.16995156
		});

		assert.strictEqual('http://example.org/obj', obj.getSemanticId());

		const actualOfferedItem = await obj.getOfferedItem();
		const expectedOfferedItem = vtpmgzzbmv;
		...
		const expectedStockLimitation = 0.16995156;
