...
test('PhysicalCharacteristic', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const gkuurpozcn = new SKOSConcept({ connector, semanticId: 'http://base.com/bwvnfetuph' });
		
		const djburbqsus = new SKOSConcept({ connector, semanticId: 'http://base.com/wxvurfhnpq' });
		const obj = new PhysicalCharacteristic({
			connector,
			unit: gkuurpozcn,
			value: 0.2959885,
			physicalDimension: djburbqsus
		});

		

		const actualUnit = await obj.getQuantityUnit();
		const expectedUnit = gkuurpozcn;
		await t.test(`#unit`, () => {
			assertSemanticEqual(actualUnit, expectedUnit);
		});

		const actualValue = obj.getQuantityValue();
		const expectedValue = 0.2959885;
		await t.test(`#value`, () => {
			assert.strictEqual(actualValue, expectedValue);
		});

		const actualPhysicalDimension = await obj.getQuantityDimension();
		const expectedPhysicalDimension = djburbqsus;
