...
test('Catalog', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const ksbzappphn = [new Enterprise({ connector, semanticId: 'http://base.com/iggrlgdfkw' })];
		const yvxkfwtiih = [new CatalogItem({ connector, semanticId: 'http://base.com/keviuqwoui' })];
		const obj = new Catalog({
			connector,
			semanticId: "http://example.org/obj",
			maintainers: ksbzappphn,
			items: yvxkfwtiih
		});
		...
		const expectedMaintainers = ksbzappphn;
		...
		const expectedItems = yvxkfwtiih;
