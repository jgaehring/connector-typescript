...
test('Catalog', async (t) => {
	await t.test('#constructor', async (t) => {
		const connector = new Connector();
		const tfdtyovudm = [new Enterprise({ connector, semanticId: 'http://base.com/navngpmqou' })];
		const huigykdrtg = [new CatalogItem({ connector, semanticId: 'http://base.com/ngjqpuhlvi' })];
		const obj = new Catalog({
			connector,
			semanticId: "http://example.org/obj",
			maintainers: tfdtyovudm,
			items: huigykdrtg
		});
		...
		const expectedMaintainers = tfdtyovudm;
		...
		const expectedItems = huigykdrtg;
